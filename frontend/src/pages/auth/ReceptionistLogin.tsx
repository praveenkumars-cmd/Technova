...
export const ReceptionistLogin: React.FC = () => {
  ...
  const onSubmit = (data: LoginFormData) => {
    ...
    showToast('Admissions Desk Access', `Welcome back, Elena Rostova!`, 'success');
    navigate('/receptionist/dashboard');
  };

  const handleQuickDemo = () => {
    setValue('email', 'elena.rostova@pulsecare.com');
    setValue('password', 'password123');
    onSubmit({ email: 'elena.rostova@pulsecare.com', password: 'password123' });
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8 transition-colors">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        <Link to="/" className="inline-flex items-center gap-3 mb-4">
          <div className="p-2.5 rounded-2xl bg-amber-600 text-white shadow-soft-sm">
            <HeartPulse className="w-7 h-7" />
          </div>
          <span className="font-extrabold text-2xl tracking-tight text-slate-900 dark:text-slate-100">
            Pulse<span className="text-amber-600 dark:text-amber-400">Care</span>
          </span>
        </Link>

        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-50 dark:bg-amber-950/80 text-amber-700 dark:text-amber-300 text-xs font-bold mb-2">
          <Building className="w-4 h-4" />
          <span>Hospital Admissions & Walk-In Desk</span>
        </div>
        ...
        <p className="mt-2 text-xs sm:text-sm text-slate-500 dark:text-slate-400">
          Manage master appointments, walk-in registrations, and staff duty rosters.
        </p>
      </div>
        ...
        <div className="bg-white dark:bg-slate-900 py-8 px-6 sm:px-10 shadow-xl rounded-3xl border border-slate-200/80 dark:border-slate-800">
          <div className="mb-6 p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs font-bold text-amber-600 dark:text-amber-400 underline hover:text-amber-800"
            >
              1-Click Demo Login
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <Input
              label="Staff Email"
              type="email"
              placeholder="elena.rostova@pulsecare.com"
            ...
            <div className="flex items-center justify-between text-xs">
              <Checkbox label="Remember desk login" {...register('rememberMe')} />
              <span className="font-semibold text-amber-600 dark:text-amber-400 cursor-pointer">Help</span>
            </div>
